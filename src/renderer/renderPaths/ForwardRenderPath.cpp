...
namespace StarryEngine {
    bool ForwardRenderPath::initialize(RHI::DescriptorSetLayoutHandle globalSetLayout) {
        m_globalSetLayout = globalSetLayout;

        m_recorder = std::make_shared<RenderGraph::MeshDrawRecorder>(m_resMgr);

        if (!createRenderGraph())return false;

        return true;
    }

    bool ForwardRenderPath::createRenderGraph() {
        // 创建 RenderGraph
    ...
    RHI::PipelineHandle ForwardRenderPath::getOrCreatePipeline(
    ...
    void ForwardRenderPath::setDrawItems(const Scene::AnalysisSceneResult& secneData) {
