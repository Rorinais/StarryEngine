...
namespace StarryEngine::RenderGraph {
    ...
    public:
        ...
        std::string addPreserve(TextureId texId);   // 保留附件无需配置
    ...
    private:
        ...
        std::unordered_map<std::string, TextureId> m_keyToTexId;           // key -> 纹理 ID（添加时记录）
        std::unordered_map<std::string, TextureId> m_attachmentKeyToTexId; // key -> 纹理 ID（编译后）
        std::unordered_map<std::string, AttachmentParams> m_keyToParams;   // key -> 参数（用于验证）
        ...
        std::vector<std::shared_ptr<ISubpassRecorder>> m_subpassRecorders;
