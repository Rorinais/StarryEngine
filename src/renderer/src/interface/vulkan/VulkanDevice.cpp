...
namespace StarryEngine {
    ...
    VulkanDevice::VulkanDevice(std::shared_ptr<VulkanInstance> instance, VkSurfaceKHR surface, const Config& config)
        : mInstance(instance)
        , mSurface(surface)
        , mConfig(config)
        , mQueueHandles(std::make_shared<VulkanQueueHandles>())
    ...
    // VMA 方式创建缓冲区
    VMABuffer VulkanDevice::createBufferWithVMA(VkDeviceSize size, VkBufferUsageFlags usage,
    ...
    void VulkanDevice::uploadDataToVmaBuffer(VkBuffer buffer, VmaAllocation allocation,
    ...
    void VulkanDevice::destroyBufferWithVMA(const VMABuffer& buffer) {
        destroyBufferWithVMA(buffer.buffer, buffer.allocation);
    }

    void VulkanDevice::destroyBufferWithVMA(VkBuffer buffer, VmaAllocation allocation) {
        if (mVmaAllocator != VK_NULL_HANDLE) {
            vmaDestroyBuffer(mVmaAllocator, buffer, allocation);
        }
    }

    void VulkanDevice::uploadDataToTraditionalMemory(VkDeviceMemory memory, const void* data,
    ...
    void VulkanDevice::uploadDataViaStagingBuffer(VkCommandPool commandPool,
    ...
    // 传统方式创建缓冲区
    VMATraditionalBuffer VulkanDevice::createBufferTraditional(VkDeviceSize size, VkBufferUsageFlags usage,
    ...
    // 传统方式创建缓冲区并上传数据
    VMATraditionalBuffer VulkanDevice::createBufferTraditionalWithData(VkCommandPool commandPool,
    ...
    // 传统方式销毁缓冲区
    void VulkanDevice::destroyBufferTraditional(const VMATraditionalBuffer& buffer) {
        destroyBufferTraditional(buffer.buffer, buffer.memory);
    }

    void VulkanDevice::destroyBufferTraditional(VkBuffer buffer, VkDeviceMemory memory) {
        vkDestroyBuffer(mLogicalDevice, buffer, nullptr);
    ...
    std::vector<VMABuffer> VulkanDevice::createVmaBuffers(uint32_t count,
        VkDeviceSize size,
    ...
    std::vector<VMATraditionalBuffer> VulkanDevice::createTraditionalBuffers(uint32_t count,
        VkDeviceSize size,
        VkBufferUsageFlags usage,
    ...
    void VulkanDevice::destroyVmaBuffers(const std::vector<VMABuffer>& buffers) {
        for (const auto& buffer : buffers) {
            destroyBufferWithVMA(buffer);
        }
    }

    void VulkanDevice::destroyTraditionalBuffers(const std::vector<VMATraditionalBuffer>& buffers) {
        for (const auto& buffer : buffers) {
    ...
    // VMA 方式创建图像
    VMAImage VulkanDevice::createImageWithVMA(
    ...
    // 销毁 VMA 图像
    void VulkanDevice::destroyImageWithVMA(const VMAImage& image) {
        destroyImageWithVMA(image.image, image.allocation);
    }

    void VulkanDevice::destroyImageWithVMA(VkImage image, VmaAllocation allocation) {
        if (mVmaAllocator != VK_NULL_HANDLE) {
    ...
    // 传统方式创建图像
    VMATraditionalImage VulkanDevice::createImageTraditional(
        uint32_t width, uint32_t height, VkFormat format,
        VkImageTiling tiling, VkImageUsageFlags usage,
        VkMemoryPropertyFlags properties,
    ...
    // 销毁传统图像
    void VulkanDevice::destroyImageTraditional(const VMATraditionalImage& image) {
        destroyImageTraditional(image.image, image.memory);
    }

    void VulkanDevice::destroyImageTraditional(VkImage image, VkDeviceMemory memory) {
        vkDestroyImage(mLogicalDevice, image, nullptr);
        if (memory != VK_NULL_HANDLE) {
            vkFreeMemory(mLogicalDevice, memory, nullptr);
            memory = VK_NULL_HANDLE;
    ...
    VkImageView VulkanDevice::createImageView(VkImage image,
        VkFormat format,
    ...
    VMAImageFull VulkanDevice::createImageWithVMAFull(
        uint32_t width, uint32_t height, VkFormat format,
        VkImageTiling tiling, VkImageUsageFlags usage,
    ...
    void VulkanDevice::destroyImageWithVMAFull(VMAImageFull& image) {
        destroyImageView(image.view);
        destroyImageWithVMA(image.image, image.allocation);
    }

    TraditionalImageFull VulkanDevice::createImageTraditionalFull(uint32_t width, uint32_t height, VkFormat format,
        VkImageTiling tiling, VkImageUsageFlags usage,
    ...
    void VulkanDevice::destroyImageTraditionalFull(TraditionalImageFull& image) {
        destroyImageView(image.view);
        destroyImageTraditional(image.image, image.memory);
    }
    ...
    VkFramebuffer VulkanDevice::createFramebuffer(VkRenderPass renderPass,
    ...
    VkRenderPass VulkanDevice::createRenderPass(const std::vector<VkAttachmentDescription>& attachments,
    ...
    void  VulkanDevice::destroyQueryPool(VkQueryPool& queryPool) {
    ...
    VkCommandBuffer VulkanDevice::allocateCommandBuffer(VkCommandPool& pool, VkCommandBufferLevel level) {
    ...
    std::vector<VkCommandBuffer> VulkanDevice::allocateCommandBuffers(VkCommandPool& pool,
        uint32_t count,
    ...
    void VulkanDevice::freeCommandBuffers(VkCommandPool& pool, const std::vector<VkCommandBuffer>& commandBuffers) {
    ...
    VkPipelineLayout VulkanDevice::createPipelineLayout(const VkPipelineLayoutCreateInfo& pipelineLayoutInfo) {
    ...
    VkPipelineLayout VulkanDevice::createPipelineLayout(const std::vector<VkDescriptorSetLayout>& setLayouts,
    ...
    VkPipeline VulkanDevice::createComputePipeline(const VkComputePipelineCreateInfo& createInfo) {
        VkPipeline pipeline;
    ...
    VkShaderModule VulkanDevice::createShaderModule(const std::vector<uint32_t>& code, const std::string& debugName) {
    ...
    VkPipelineShaderStageCreateInfo VulkanDevice::createShaderStageInfo(
        VkShaderModule module,
    ...
    VkDescriptorPool VulkanDevice::createDescriptorPool(const std::vector<VkDescriptorPoolSize>& poolSizes,
        uint32_t maxSets) {
    ...
    void VulkanDevice::updateDescriptorSet(VkDescriptorSet set,
    ...
    void VulkanDevice::blockFence(VkFence& fence,uint64_t timeout) {
        if (fence != VK_NULL_HANDLE) {
    ...
    VkSampler VulkanDevice::createSampler(VkFilter magFilter, VkFilter minFilter,
    ...
    void VulkanDevice::submitAsync(VkCommandBuffer commandBuffer, VkFence fence) {
        vkEndCommandBuffer(commandBuffer);
        ...
        vkQueueSubmit(mQueueHandles->getGraphicsQueue(), 1, &submitInfo, fence);
        // 不 vkQueueWaitIdle、不释放命令缓冲：fence 信号后由持有方重录复用。
    }
    ...
    void VulkanDevice::getLayoutTransitionInfo(
        VkImageLayout oldLayout,
        VkImageLayout newLayout,
    ...
    void VulkanDevice::setBufferName(VkBuffer buffer, const char* name) {
        setObjectName(reinterpret_cast<uint64_t>(buffer), VK_OBJECT_TYPE_BUFFER, name);
    }

    void VulkanDevice::setImageName(VkImage image, const char* name) {
    ...
    VkPhysicalDevice VulkanDevice::selectPhysicalDevice(VkSurfaceKHR surface) {
    ...
    int32_t VulkanDevice::ratePhysicalDevice(VkPhysicalDevice device, VkSurfaceKHR surface) {
    ...
    bool VulkanDevice::isPhysicalDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface) {
    ...
    void VulkanDevice::createLogicalDevice() {
        ...
        if (mQueueFamilyIndices.graphicsFamily.has_value()) {
            ...
            if (graphicsQueue != VK_NULL_HANDLE) {
                mQueueHandles->addQueue(VulkanQueueHandles::QueueType::Graphics,
                    graphicsQueue,
        ...
        if (mQueueFamilyIndices.presentFamily.has_value()) {
            ...
            if (presentQueue != VK_NULL_HANDLE) {
                mQueueHandles->addQueue(VulkanQueueHandles::QueueType::Present,
                    presentQueue,
        ...
        if (mQueueFamilyIndices.computeFamily.has_value() &&
            ...
            if (computeQueue != VK_NULL_HANDLE) {
                mQueueHandles->addQueue(VulkanQueueHandles::QueueType::Compute,
                    computeQueue,
        ...
        if (mQueueFamilyIndices.transferFamily.has_value() &&
            ...
            if (transferQueue != VK_NULL_HANDLE) {
                mQueueHandles->addQueue(VulkanQueueHandles::QueueType::Transfer,
                    transferQueue,
    ...
    uint32_t VulkanDevice::getQueueFamilyIndex(uint32_t queueType) const {
        switch (queueType) {
        case 0:
    ...
    // 获取设备类型字符串
    const char* VulkanDevice::getTypeString() const {
    ...
    // 获取供应商名称
    const char* VulkanDevice::getVendorName() const {
