...
namespace StarryEngine {
    ...
    public:
        ...
        static Ptr create(std::shared_ptr<VulkanInstance> instance, VkSurfaceKHR surface, const Config& config) {
            return std::make_shared<VulkanDevice>(instance, surface, config);
        }

        VulkanDevice(std::shared_ptr<VulkanInstance> instance, VkSurfaceKHR surface, const Config& config);
        ~VulkanDevice();
        ...
        // === 队列管理 ===
        std::shared_ptr<VulkanQueueHandles> getQueueHandles() { return mQueueHandles; }
        ...
        void submitAsync(VkCommandBuffer commandBuffer, VkFence fence);
        ...
        // ==================== 查询功能 ====================
        std::shared_ptr<VulkanInstance> getInstance() const { return mInstance; }
        ...
        uint32_t getSupportedApiMinor() const { return VK_VERSION_MINOR(mProperties.apiVersion); }
        uint32_t getRequestedApiMajor() const { return VK_VERSION_MAJOR(mInstanceApiVersion); }
        uint32_t getRequestedApiMinor() const { return VK_VERSION_MINOR(mInstanceApiVersion); }
        uint32_t getMaxUniformBufferSize() const { return mProperties.limits.maxUniformBufferRange; }
        uint32_t getMaxStorageBufferSize() const { return mProperties.limits.maxStorageBufferRange; }
        VkDeviceSize getMinUniformBufferOffsetAlignment() const { return mProperties.limits.minUniformBufferOffsetAlignment; }
        uint32_t getMaxImageDimension2D() const { return mProperties.limits.maxImageDimension2D; }
        uint32_t getMaxImageDimension3D() const { return mProperties.limits.maxImageDimension3D; }
        uint32_t getMaxImageArrayLayers() const { return mProperties.limits.maxImageArrayLayers; }
        uint32_t getMaxDescriptorSets() const { return mProperties.limits.maxBoundDescriptorSets; }
        uint32_t getMaxSamplerAllocationCount() const { return mProperties.limits.maxSamplerAllocationCount; }
        uint32_t getMaxPushConstantsSize() const { return mProperties.limits.maxPushConstantsSize; }
        uint32_t getMaxComputeWorkGroupInvocations() const { return mProperties.limits.maxComputeWorkGroupInvocations; }
        uint32_t getMaxFragmentOutputAttachments() const { return mProperties.limits.maxFragmentOutputAttachments; }
        ...
        bool supportsSampleRateShading() const { return mPhysicalDeviceFeatures.sampleRateShading; }

        // ==================== 队列信息 ====================
        uint32_t getGraphicsQueueFamilyIndex() const {
            return mQueueFamilyIndices.graphicsFamily.value_or(VK_QUEUE_FAMILY_IGNORED);
        }
        ...
        bool supportsExtension(const std::string& extension) const {
            return mSupportedExtensions.find(extension) != mSupportedExtensions.end();
        }
    ...
    private:
        ...
        std::shared_ptr<VulkanQueueHandles> mQueueHandles;
