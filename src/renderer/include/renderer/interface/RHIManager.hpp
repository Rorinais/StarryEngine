...
namespace StarryEngine::RHI {
    ...
    public:
        ...
        // 帧缓冲：renderPass/attachments 由调用方传句柄，本层解析后交给工厂
        FramebufferHandle createFramebuffer(
            RenderPassHandle renderPass,
            const std::vector<TextureHandle>& attachments,
    ...
    private:
       ...
       // mutable std::mutex statsMutex_;
        Statistics stats_;
        mutable std::atomic<size_t> peakResourceCount_{ 0 }; 
        mutable std::atomic<size_t> peakMemoryUsage_{ 0 };   
    ...
    struct HandleTraits<RHI::FramebufferHandle> {
        ...
        static RHI::FramebufferHandle create(RHI::ResourceManager* mgr, const RHI::FramebufferDesc& desc,
            const std::string& name = "", const std::string& debugTag = "") {
            // desc 不再含句柄；调用方需显式传 renderPass/attachments。
            // 若 desc 曾携带句柄，此处需改为从 desc 读取（当前 desc 已无这些字段）。
            assert(false && "FramebufferHandle::create requires explicit renderPass/attachments");
            return RHI::FramebufferHandle::Null();
        }
